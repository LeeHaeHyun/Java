			...
			public void actionPerformed(ActionEvent e) {
				...
				if(comboBox2.getSelectedItem().toString().equals("제조업체 현황")) {
					try {
						...
						connection = DriverManager.getConnection("jdbc:oracle:thin:@211.200.18.59:1521:xe", "scott", "TIGER");
						java.sql.Statement statement = connection.createStatement();
						
						ResultSet result = statement.executeQuery("select 유형, 별명_ID, 연락처 from 회원DB");
						
						String col = "[ 유형  >> " + "별명_ID  >>  " + "연락처 ]";
						list.add(col);
						list.add(" ");
				
						while(result.next()) {
							String manufacturer =  result.getNString("유형");
							String produt_name = result.getNString("별명_ID");
							String owner = result.getNString("연락처");
